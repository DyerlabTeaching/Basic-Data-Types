...
::: {.fragment .fade-in}
You can also view the value in a variable by looking in the *Environment* tab
:::
...
```{r}
adult_beverage_time <- mdy_hms("08/20/2026 7:30:00")
adult_beverage_time
```
...
&nbsp;
```{r}
tz( adult_beverage_time ) <- "US/Eastern"
adult_beverage_time
```
:::
...
```{r}
now() - mdy_hm("10/14/1969 04:20", tz="US/Pacific") -> daysAlive
daysAlive
```
...
## When Is Dyer 21,000 Days Old?

```{r}
mdy("10/14/1969") + days(21000) -> days21K
```
&nbsp;

![](media/format.png)

On `r format(days21K,"%A, %B %d, %Y")` Dyer will be 21,000 days old. 🍻
