...
> Numerical data contains all numerical representations.
...
You have the *exponential* and *root*:
...
The *logarithms* (natural, base 10, and arbitrary bases):
...
And the *modulus operator*, which is the remainder after division:

```{r}
y %% x
```

If you didn't know what this one is, don't worry.  The modulus is just the *remainder after division* like you did in grade school.  The above code means that *23 divided by 10 has a remainder of 3*.  I include it here just to highlight the fact that many of the operators that we will be working with in `R` are created by more than just a single symbol residing at the top row of your computer keyboard.  There are just too few symbols on the normal keyboard to represent the breadth of operators.  The authors of `R` decided that using combinations of symbols to handle a broader array of operators than are available on the common keyboard using this 'wrapped' context.  This is a convention that you will get used to using.
...
<div class="box-yellow">In this case `class` is the name of the function and there are one or more things we pass to that function.  These **must** be enclosed in the parentheses associated with `class`.  The parentheses **must** be *right next to* the name of the function. If you put a space between the word `class` and the parentheses, it may not work the way you would like it to.  You've been warned.

The stuff inside the parentheses are called *arguments* and are the data that we pass to the function itself. In this case we pass a value or variable to the `class` function and it does its magic and tells us what kind of data type it is.  Many functions have several arguments that can be passed to them, some optional, some not.  We will get more into that on the lecture covering [Functions](https://dyerlab.github.io/ENVS-Lectures/functions/slides.html).</div>
...
The data type `character` is intended to represent textual data such as *actual texts*, names of objects, and other content that is intended to help both you and the audience you are trying to reach better understand your data.
...
The length of a string is a measure of how many variables there are, not the number of characters within it.  For example, the length of `name` is
...
Length is defined specifically on the number of elements in a vector, and technically the variable `name` is a vector of length one.  If we concatenate them into a vector (go see the vector content)
...
The `paste()` function is designed to take a collection of `character` variables and smush them together.  By default, it inserts a space between each of the variables and/or values passed to it.
...
Coercion of a variable that is not a `logical` into a `Logical` is more case-specific.
...
1. There are many types of calendars, we use the **Gregorian** calendar.  However, there are many other calendars that are in use that we may run into.  Each of these calendars has a different starting year (e.g., in the Assyrian calendar it is year 6770, it is 4718 in the Chinese calendar, 2020 in the Gregorian, and 1442 in the Islamic calendar).
...
3. On this planet, we have 24 different time zones. Some states (looking at you Arizona) don't feel it necessary to follow the other states around so they may be the same as PST some of the year and the same as MST the rest of the year.  The province of Newfoundland decided to be half-way between time zones so they are UTC−3:30. Some states have more than one time zone even if they are not large in size (hello Indiana).
...
Fortunately, some smart programmers have figured this out for us already.  What they did was make the second as the base unit of time and designated 00:00:00 on 1 January 1970 as the *unix epoch*.  Time on most modern computers is measured from that starting point.  It is much easier to measure the difference between two points in time using the seconds since unix epoch *and then* translate it into one or more of these calendars than to deal with all the different calendars each time. So under the hood, much of the date and time issues are kept in terms of epoch seconds.  
...
`R` has some basic date functionality built into it.  One of the easiest ways to get a date object created is to specify a date as a character string and then coerce it into a date object.  By default, this requires us to represent the date objects as "YEAR-MONTH-DAY" with padding `0` values for any integer of month or date below 9 (e.g., must be two-digits).

So for example, we can specify a date object as:

```{r}
class_start <- as.Date("2026-08-18")
class_start
```
...
If you want to make a date from a different format, you must specify which elements of the date/time object you want represented within the string using specific formatting codes.  These codes (and many more) can be found by looking at `?strptime`.  These are recognized standards that are used world-wide for these items.

```{r}
class_end <- as.Date( "12/03/26", format = "%m/%d/%y")
class_end
```
...
We can easily filter the content within a `data.frame` using some helper functions such as `hour()`, `minute()`, `wday()`, etc.  Here are some examples including pulling out the weekends.

```{r}
weekends <- df[ df$Weekday %in% c("Saturday","Sunday"), ]
weekends
```
Finding items that are in the past (past being defined as the last time this document was knit).
...
And total range of values in days using normal arithmetic operations such as the minus operator.
